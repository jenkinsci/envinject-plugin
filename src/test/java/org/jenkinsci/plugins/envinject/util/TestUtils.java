...
public class TestUtils {
    ...
    public static void saveConfigurationAs(@NonNull JenkinsRule jenkins, @NonNull FreeStyleProject project, @NonNull String userId) throws Exception {
        try (JenkinsRule.WebClient w = jenkins.createWebClient().login(userId)) {
            ...
            formByName.getInputsByName("oldScript").forEach(
                    input -> input.setValue("different value to force behavior when modified"));
