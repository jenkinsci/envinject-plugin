...
public class EnvInjectEnvVars implements Serializable {
    ...
    public Map<String, String> executeAndGetMapGroovyScript(
        ...
        logger.info("Using a deprecated code path, there might be issues.");
        SecureGroovyScript script = new SecureGroovyScript(scriptContent, false, null).configuring(ApprovalContext.create());
     ...
     * @return the map
     * @throws EnvInjectException for any exceptions generated by the script
    ...
    public Map<String, String> executeGroovyScript(
        ...
        try {
            groovyResult = script.evaluate(Jenkins.getInstance().getPluginManager().uberClassLoader, binding);
        } catch (Exception e) {
            throw new EnvInjectException(e);
        }
